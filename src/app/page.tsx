/**
 * Finance Command Center — what a CFO or Controller opens first.
 *
...
import { canonical } from '@/workflows/engine';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Finance Command Center</h1>
          <p className="mt-1 text-sm text-[var(--color-muted)]">
            {model.company.name} · close {V0_CONFIG.closeDate} · prior comparison{' '}
      ...
      <Card title="Portfolio" subtitle="Draft management analysis across six active projects">
        <div className="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-6">
          ...
          <Metric
            ...
            sub={`${portfolio.profitMovement < 0 ? '▼' : '▲'} ${usd(Math.abs(portfolio.profitMovement), { compact: true })} vs prior close`}
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <Card
          ...
          subtitle="A project is not close-ready while any blocking item is unresolved"
        >
                ...
                {metrics.map((m) => {
                  ...
                  return (
                    <tr key={m.projectId} className="hover:bg-[var(--color-canvas)]">
                      ...
                      >
                        {marginMove === null ? 'n/a' : `${marginMove > 0 ? '+' : ''}${marginMove.toFixed(2)}`}
                      </Td>
                      <Td
                        align="right"
                        className={
          ...
          <p className="mt-3 text-xs text-[var(--color-muted)]">
            Billing position: negative is underbilled. Figures in brackets are blocking items.
          </p>
        </Card>

        <div className="space-y-6">
          <Card title="Waiting on" subtitle={`${openTasks.length} open items, ${blockingTasks.length} blocking`}>
            {waitingOn.size === 0 ? (
          ...
          <Card title="Cutoff candidates" subtitle="Received but not yet invoiced">
